...
{
    public partial class frmServiceCenter : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                string qry = "insert into ServiceCenter (Name,Address,ContactNo,ContactPerson,Email,Fax,Deleted) values (@Name,@Address,@ContactNo,@ContactPerson,@Email,@Fax,@Deleted)";
                ...
                using (SqlConnection con = new SqlConnection(cs))
                {
                    using (SqlCommand cmd = new SqlCommand(qry, con))
                    {
                        ...
                        con.Close();
                        dgvServiceCenterfresh();
                        if (k > 0)
                        {
                            MessageBox.Show("Record added sucessfully.","Save",MessageBoxButtons.OK,MessageBoxIcon.Information);
                            txtSCID.Text = txtName.Text = txtAddress.Text = txtContactNo.Text = txtContactPerson.Text = txtEmail.Text = txtFax.Text = string.Empty;
                        }
                        else
                        {
        ...
        private void dgvServiceCenterfresh()
        {
            string qry = "select SCID, Name, Address, ContactNo, ContactPerson, Email, Fax from ServiceCenter where deleted=0";
            ...
            da.Fill(tbl);
            dgvServiceCenter.DataSource = tbl;
            con.Close();
            dgvServiceCenter.Columns[2].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
            dgvServiceCenter.AllowUserToAddRows = false;
            dgvServiceCenter.ClearSelection();
            dgvServiceCenter.Columns[0].Width = 60;
            dgvServiceCenter.Columns[1].Width = 150;
        ...
        private void dgvServiceCenter_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex != -1)
                {
                    ...
                    DataGridViewRow dgvRow = dgvServiceCenter.Rows[e.RowIndex];

                    scid = Convert.ToInt32(dgvRow.Cells[0].Value.ToString()); // how i extract SCID for sql queries, it should be a public int
                    txtSCID.Text = dgvRow.Cells[0].Value.ToString();
        ...
        {
            dgvServiceCenterfresh();
            dgvServiceCenter.RowHeadersVisible = false;



            frmGarage fg = new frmGarage();
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            {
                string qry = " update ServiceCenter set Name = @Name ,  Address=@Address, ContactNo=@ContactNo, ContactPerson=@ContactPerson , Email=@Email, Fax=@Fax where   SCID  = @SCID  ";
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                string qry = " update ServiceCenter set Updated = @Updated where   SCID  = @SCID  ";
                SqlConnection con = new SqlConnection(cs);
                SqlCommand cmd = new SqlCommand(qry, con);
                ...
                cmd.Parameters.AddWithValue("@SCID", scid);
                cmd.Parameters.AddWithValue("@Updated",1);
                cmd.ExecuteNonQuery();
